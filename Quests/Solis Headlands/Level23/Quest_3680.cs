...
namespace AutoExp.Quests
{
    internal class Quest_3680 : Quest
    {
        ...
        public override bool RunQuest(Host host)
        {
            ...
            {
                Zone zone = new RoundZone(16325.24, 8139.72, 120);
                if (!host.movementModule.GpsMove("Quest_3680_1"))
                    return false;
                host.farmModule.SetFarmMobsFromDoodads(zone, new uint[] { 9888 }, new uint[] {9885 });
                while ((host.farmModule.farmState == Modules.FarmState.MobsFromDoodads || host.farmModule.farmState == Modules.FarmState.Enabled) && quest.status == QuestStatus.Accepted)
                {
