...
{
    internal class Quest_873 : Quest
    {
        public Quest_873(int minLvl, int maxLvl, QuestRace race, uint[] reqQuests)
            : base(873, minLvl, maxLvl, race, reqQuests)
        { }
        ...
        public override bool RunQuest(Host host)
        {
            ...
            if (getQuest() == null)
            {
                if (!host.movementModule.GpsMove("Mahadevi_Sabri")) return false;
                Thread.Sleep(1000);
                host.StartQuest(id);
                Thread.Sleep(1000);
            }
