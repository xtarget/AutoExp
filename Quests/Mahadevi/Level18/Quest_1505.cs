...
{
    internal class Quest_1505 : Quest
    {
        public Quest_1505(int minLvl, int maxLvl, QuestRace race, uint[] reqQuests)
            : base(1505, minLvl, maxLvl, race, reqQuests)
        { }
        ...
        public override bool RunQuest(Host host)
        {
            ...
            if (quest != null && quest.status == QuestStatus.Accepted)
            {
                ...
                if (!zone.ObjInZone(host.me))
                    if (!host.movementModule.GpsMove("Quest_1505_1")) return false;
