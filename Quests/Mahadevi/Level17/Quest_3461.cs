        ...
        public override bool RunQuest(Host host)
        {
            ...
            if (quest.status == QuestStatus.Accepted)
            {
                try
                {
                    ...
                    while (host.farmModule.readyToActions && host.farmModule.farmState == Modules.FarmState.Enabled && getNearest() != null)
                        Thread.Sleep(100);
                    host.farmModule.StopFarm();
                    Thread.Sleep(1000);
                    host.farmModule.UseDoodadSkill(16805, host.getNearestDoodad(10492), true);
                    Thread.Sleep(1000);
                }
                finally
                {
