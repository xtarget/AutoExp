...
{
    internal class Quest_858 : Quest
    {
        public Quest_858(int minLvl, int maxLvl, QuestRace race, uint[] reqQuests)
            : base(858, minLvl, maxLvl, race, reqQuests)
        { }
        ...
        public override bool RunQuest(Host host)
        {
            ...
            {
                Zone zone = new RoundZone(20171.15, 8358.97, 45);
                if (!zone.ObjInZone(host.me))
                    if (!host.movementModule.GpsMove("Quest_861_1")) return false;
                host.farmModule.SetFarmDoodads(zone, new uint[] { 7134 });
