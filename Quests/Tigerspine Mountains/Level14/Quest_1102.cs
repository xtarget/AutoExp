...
namespace AutoExp.Quests
{
    internal class Quest_1102 : Quest
    {
        ...
        private Creature getNearestMob(Zone zone)
        {
            ...
            {
                if (creature.creatureId == 3364 && host.isAlive(creature) && zone.ObjInZone(creature) && host.buffTime(941) == 0)
                {
        ...
        public override bool RunQuest(Host host)
        {
            ...
            if (quest != null && quest.status == QuestStatus.Accepted)
            {
                ...
                while (host.farmModule.readyToActions && quest.status == QuestStatus.Accepted && host.me.isAlive())
                {
                    ...
                    if (m != null)
                    {
                        host.SetTarget(m);
                        Thread.Sleep(500);
                        host.farmModule.UseItemAndWait(13965);
                        Thread.Sleep(1000);
                        while (host.farmModule.readyToActions && host.me.isAlive() && host.farmModule.aggroMobsCount() > 0)
                            Thread.Sleep(100);
                        Thread.Sleep(4000);
