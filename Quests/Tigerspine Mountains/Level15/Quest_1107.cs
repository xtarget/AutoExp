...
namespace AutoExp.Quests
{
    internal class Quest_1107 : Quest
    {
        ...
        public override bool RunQuest(Host host)
        {
            ...
            if (host.me.charRace == CharRace.Hariharan && !checkQuestCompletedOrPerfomed(3538))
                return false;
                ...
                while (host.farmModule.readyToActions && host.me.isAlive() && quest.status == QuestStatus.Accepted)
                {
                    if (host.farmModule.aggroMobsCount() == 0)
                    {
                        ...
                        if (m != null)
                        {
                            host.SetTarget(m);
                            Thread.Sleep(1000);
                            host.farmModule.UseItemAndWait(14579);
                            Thread.Sleep(2500);
                        }
                        Thread.Sleep(1000);
                        host.farmModule.UseDoodadSkill(12052, host.getNearestDoodad(3075), true);
                    }
                    else 
                    {
