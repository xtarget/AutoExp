        ...
        public override bool RunQuest(Host host)
        {
            ...
            if (quest != null && quest.status == QuestStatus.Accepted)
            {
                ...
                if (host.itemCount(5140) < 3)
                {
                    ...
                    while (host.farmModule.readyToActions && host.me.isAlive() && quest.status == QuestStatus.Accepted && host.itemCount(5140) < 3)
                        Thread.Sleep(100);
                    host.farmModule.StopFarm();
                }
                if (host.itemCount(5140) >= 3)
                {
                    while (host.farmModule.readyToActions && host.me.isAlive() && quest.status == QuestStatus.Accepted && host.itemCount(5140) > 0)
                    {
                        ...
                        if (c != null && zone.ObjInZone(c) && host.getBuff(c,1439) != null)
                        {
                            host.SetTarget(c);
                            Thread.Sleep(500);
                            host.farmModule.UseItemAndWait(5140);
                        }
                        Thread.Sleep(100);
                    }
                }
