        ...
        public override bool RunQuest(Host host)
        {
            ...
            if (quest != null && quest.status == QuestStatus.Accepted)
            {
                ...
                if (c != null)
                {
                    ...
                    host.TurnDirectly();
                    Thread.Sleep(500);
                    host.farmModule.UseItemAndWait(18428);
                    Thread.Sleep(2000);
                    Zone zone = new RoundZone(24057.10, 11471.73, 10);
                    host.farmModule.SetFarmMobs(zone, new uint[] { 3315 });
                    while (host.farmModule.readyToActions && host.farmModule.farmState == Modules.FarmState.Enabled && quest.status == QuestStatus.Accepted && host.farmModule.aggroMobsCount() > 0)
                        Thread.Sleep(100);
