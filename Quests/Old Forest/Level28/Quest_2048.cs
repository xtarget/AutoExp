            ...
            if (quest != null && quest.status == QuestStatus.Accepted)
            {
                if (quest.steps[1] == 0)
                {
                    if (!host.movementModule.GpsMove("Quest_2048_2"))
                        return false;
                    Thread.Sleep(1000);
                    host.farmModule.UseDoodadSkill(13683, host.getNearestDoodad(5210), true);
                    Thread.Sleep(1000);
                ...
                if (quest.steps[0] == 0)
                {
                    ...
                    while (host.farmModule.readyToActions && host.farmModule.farmState == Modules.FarmState.Enabled && quest.steps[0] == 0 && quest.status == QuestStatus.Accepted && mobsCountInZone(zone, new uint[] { 3110, 3126 }) > 0)
                        Thread.Sleep(100);
                    host.farmModule.StopFarm();
                    Thread.Sleep(1000);
                    host.farmModule.UseDoodadSkill(13683, host.getNearestDoodad(5207), true);
                    Thread.Sleep(1000);
                }
                if (quest.steps[2] == 0)
                {
                    if (!host.movementModule.GpsMove("Quest_2048_3"))
                        return false;
                    Thread.Sleep(1000);
                    host.farmModule.UseDoodadSkill(13683, host.getNearestDoodad(5213), true);
                    Thread.Sleep(1000);
                    host.StartQuest(3908);
                    Thread.Sleep(1000);
                }
