...
{
    internal class Quest_1140 : Quest
    {
        public Quest_1140(int minLvl, int maxLvl, QuestRace race, uint[] reqQuests)
            : base(1140, minLvl, maxLvl, race, reqQuests)
        { }
        ...
        public override bool RunQuest(Host host)
        {
            ...
            if (quest != null && quest.status == QuestStatus.Accepted)
            {
                ...
                if (!host.movementModule.GpsMove("Quest_1140_1"))
                    return false;
                host.farmModule.SetFarmMobs(zone, new uint[] { 2213 });
            ...
            if (!checkQuestCompletedOrPerfomed(1134))
                return false;
            if (!checkQuestCompletedOrPerfomed(1138))
                return false;
